		...
		provideInlineCompletionItems: async function (
			...
			return new Promise((resolve) => {
				...
				debounce_timer = setTimeout(async () => {
					...
					const prefix_range = new vscode.Range(
						new vscode.Position(0, 0),
						position,
					);
					const prefix_text = document.getText(prefix_range);

					const suffix_range = new vscode.Range(
						position,
					...
					const workspace_root = workspace_folders
					...
					try {
						...
						if (
							...
							response_data.suggestion.trim().length > 0
						) {
							// Print to the debug console so we can see what Ollama actually returned
							console.log(
								`[Axiom Debug] Model returned: ${response_data.suggestion}`,
							);
							return resolve([
						...
						vscode.window.showErrorMessage(
							`Axiom Inference Error: ${error.message}`,
						);
						resolve(undefined);
					}
